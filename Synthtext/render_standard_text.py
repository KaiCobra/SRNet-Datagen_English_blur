...
#### Blur ####
def make_standard_text2(font_path, text, shape, padding=0.1, color=(0, 0, 0), init_fontsize=25, blur=True, blur_kernel_size=(5, 5)):
    font = freetype.Font(font_path)
    font.antialiased = True
    font.origin = True
    fontsize = init_fontsize
    ...
    # 如果需要模糊化，對畫布進行模糊處理
    if blur:
        canvas_rgb = cv2.GaussianBlur(canvas_rgb, blur_kernel_size, 0)
