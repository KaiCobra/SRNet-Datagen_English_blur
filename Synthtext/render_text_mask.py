...
def render_normal(font, text):
    ...
    print(lines[np.argmax(lengths)])
    # initialize the surface to proper size:
    # print(font.get_rect(lines[np.argmax(lengths)])
    ...
    for l in lines:
        ...
        for ch in l: # render each character
            if ch.isspace(): # just shift
                ...
                ch_highet = font.size*0.06
                ch_space = font.size*0.05
                #if ch == '·':
                #    y -= ch_highet
                if ch == '1':
                    x += font.size*0.12
...
def perspective(img, rotate_angle, zoom, shear_angle, perspect, pad): # w first
    ...
    #resimg = np.zeros((64 + pad[2] + pad[3], 64 + pad[0] + pad[1])).astype(np.uint8)
    resimg[pad[2]:pad[2]+text_h, pad[0]:pad[0]+text_w] = canvas[miny:maxy+1, minx:maxx+1]
    #resimg[pad[2]+15:pad[2]+text_h+15, pad[0]+15:pad[0]+text_w+15] = canvas[miny:maxy+1, minx:maxx+1]
    return resimg
